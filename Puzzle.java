...
public class Puzzle {
    ...
    public Puzzle() {
        ...
        // fields that you add.
        this.isRow = new boolean[DIM][DIM+1];
        this.isCol = new boolean[DIM][DIM+1]; 
    }
    ...
    private boolean solveRB(int n) {
        ...
        if(!valIsFixed[row][col]){
            for(int val = 1; val<=9; val++){
                if(this.subgridHasValue[row/SUBGRID_DIM][col/SUBGRID_DIM][val] == false && this.isRow[row][val] == false&&this.isCol[col][val] == false){
